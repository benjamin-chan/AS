...
data indRx(keep=&idVar &IndexDateVarName &CovariateListRx);
  ...
  if lowcase(etc_name)=:'dm'        then dm=1;
  if lowcase(etc_name)=:'folate'    then folate=1;
  if lowcase(etc_name)=:'fungus'    then fungus=1;
  if lowcase(etc_name)=:'htn'       then htn=1;
  if lowcase(etc_name)=:'naproxen'  then naproxen=1;
  if lowcase(etc_name)=:'narcotics' then narcotics=1;
  if lowcase(etc_name)=:'nsaid'     then nsaid=1;
  
  if lowcase(etc_name)=:'op_ppi'    then op_PPI=1;
  if lowcase(etc_name)=:'op_bis'    then op_bisphosp=1;
  
  if lowcase(etc_name)=:'op_other'  then op_other=1;
  if lowcase(etc_name)=:'sedative'  then sedative=1;
  if lowcase(etc_name)=:'steroid'   then steroid=1;
  if lowcase(etc_name)=:'tb'        then tb=1;
  if lowcase(etc_name)=:'thyroid'   then thyroid=1;
  if lowcase(etc_name)=:'viral'     then viral=1;
  if lowcase(etc_name)=:'dementia'  then dementia=1; 
  if lowcase(etc_name)=:'statin'    then statin=1;
  if lowcase(etc_name)=:'nsaidcox'  then nsaidcox=1;
  if lowcase(etc_name)=:'bblocker'  then BBlocker=1; 
  if lowcase(etc_name)=:'antithrom' then antithrom=1;  
  if lowcase(etc_name)=:'antibioti' then antibiotics=1; 
