...
%MACRO RULEOUT(SETIN,PATID,CLMDTE,START,FINISH,DXVARSTR,NDXVAR,HCPCS,FILETYPE);
  ...
  proc sort data=WRK.CLMRECS;
    by &PATID &CLMDTE;

  /* Separate data into variable of interest and variables unaffected by this macro */
  data WRK.test(keep=&PATID &CLMDTE icd9dx cnt j &filetype &NDXVAR) WRK.CLMRECS(drop=j cnt &CLMDTE icd9dx &DXVARSTR);
    set WRK.CLMRECS;
    by &PATID &CLMDTE;
    ...
    do j=1 to &NDXVAR;
      icd9dx=dxcode(j);
      if not (icd9dx='     ') then output WRK.test;
      end;

  proc sort data=WRK.test;
    by &PATID icd9dx &FILETYPE &CLMDTE;

  /* Initial pass at Dx codes: in WRK.test, retain all code in MEDPAR and those that appear */
  /* multiple times in a time span exceeding 30 days.  In WRK.hold, retain all codes that   */
  /* appear multiple time, but exact time span unclear when record processed.  In WRK.icds, */
  /* retain all patient/icd combinations that are discovered to be non-ruleout.         */
  data WRK.test(keep=&PATID clm_dtea icd9dx cnta ja &NDXVAR) WRK.icds(keep=&PATID icd9dx iflag mflag &NDXVAR)
       WRK.hold(keep=&PATID clm_dtea icd9dx cnta ja &NDXVAR);
    set WRK.test;
    by &PATID icd9dx &FILETYPE &CLMDTE;
    ...
    if &FILETYPE='M' then do;               /* KEEP, in medpar */
      ...
      if ((&CLMDTE-clm_dtea) > 30) | ((&CLMDTE-clm_dtef) > 30) | ((clm_dtel- &CLMDTE) > 30) then do; /* KEEP, outside 30 days */
        output WRK.test;                        /* KEEP previous information */
        clm_dtea=&CLMDTE;
        cnta=cnt;
        ja=j;
        output WRK.test;                        /* KEEP current information */
        iflag=1;
        output WRK.icds;
        end;
      ...
      if ((&CLMDTE-clm_dtea) > 30) | ((&CLMDTE-clm_dtef) > 30) | ((clm_dtel- &CLMDTE) > 30) then do; /* KEEP, outside 30 days */
        ...
        output WRK.hold;                        /* HOLD previous information */
  ...
  proc sort data=WRK.icds nodups;
    by &PATID icd9dx;

  proc sort data=WRK.hold;
    by &PATID icd9dx;

  /* find all patient/icd9dx combinations that were held for recheck and were found to be valid */
  data WRK.test2(drop=iflag mflag);
    merge WRK.hold(in=h) WRK.icds(in=i);
    by &PATID icd9dx;
    if h;
    if iflag > 0 or mflag > 0 then output WRK.test2;

  data WRK.test;
    set WRK.test WRK.test2;
  ...
  data WRK.test(keep=&PATID cnta clm_dtea &DXVARSTR);
    set WRK.test;
    by &PATID cnta ja;
    ARRAY DXCODE /* (&NDXVAR) */ $ &DXVARSTR;
    retain &DXVARSTR '     ';
  ...
  data WRK.CLMRECS(drop=cnta);
    merge WRK.test(in=t) WRK.CLMRECS(in=e);
    by &PATID cnta clm_dtea;
    if e;
