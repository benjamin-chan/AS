...
data uab_dx_720;
set 
MPSTD.dx_07_10 MPSTD.dx_ip_07_10 MPSTD.dx_op_07_10;
format DISCHARGE_STATUS ADMIT_TYPE POA PROV_TYPE FACILITY_TYPE;
where dx=:"720";
run;
...
select count(distinct patid) from DT.cohortASTDMPCD;
select count(distinct patid) from cohortASTDMPCD;
...
select * from cohort2
where patid not in (select distinct patid from DT.cohortASTDMPCD);

select * from DT.cohortASTDMPCD
...
proc sort data=BioMPSTD nodupkey; by patid gnn DISPENSE_DATE;run;
...
ods html close;
ods html;
proc freq data=BioMPSTD; tables gnn;run;
...
ods html close;
ods html;
proc freq data=dt.DmardMPSTD;
tables gnn;
run;
proc freq data=DmardMPSTD;
tables gnn;
run;
...
;
quit;
proc sort data=NsaidMPSTD nodupkey; by patid gnn DISPENSE_DATE;run;

proc freq data=dt.NSAIDMPSTD;
tables gnn;
run;
...
proc sql;
  create table indexLookup AS
    ...
    select "MPCD"       as database, "DMARD" as exposure, &var from ASDMARDCohortMPStd_ex2  union corr
    select "MPCD"       as database, "NSAID" as exposure, &var from ASNSAIDCohortMPStd_ex2  union corr
    select "MPCD"       as database, "No exposure" as exposure, &var from ASNoExpCohortMPStd_ex2  ;
