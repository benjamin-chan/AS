...
set1 <- c("Cancer", "Cardiac disease", "Infection")
set2 <- c("Inflammatory bowel disease", "Kidney disease", "Lung disease")
set3 <- c("Neurological Disease", "Osteoporotic fracture", "PsO/PsA", "Uveitis")
dfLong %>% 
  filter(outcomeCategory %in% set1) %>% 
  plotIncidence(filename = "incidenceTNFvsNSAID_1")
dfLong %>% 
  filter(outcomeCategory %in% set2) %>% 
  plotIncidence(filename = "incidenceTNFvsNSAID_2")
dfLong %>% 
  filter(outcomeCategory %in% set3) %>% 
